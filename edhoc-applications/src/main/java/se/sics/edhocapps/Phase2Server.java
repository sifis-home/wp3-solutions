...
/**
 * EDHOC server using Authentication method 3, and not using the Optimized
 * request.
 *
 */
public class Phase2Server extends CoapServer {
	...
	// Set to True if this CoAP server is the EDHOC responder (only flow
	...
	// The type of the credential identifier to use as ID_CRED_R in EDHOC
	// message_2 or as ID_CRED_I in EDHOC message_3, i.e., for this peer to
	// indicate its own authentication credential to the other peer. Possible
	// values: ID_CRED_TYPE_KID ; ID_CRED_TYPE_CWT ; ID_CRED_TYPE_CCS ;
	...
	public static void main(String[] args) {
		...
		System.out.println("Phase2Server starting on port: " + COAP_PORT);
	...
	public Phase2Server() {
		...
		EdhocEndpointInfo edhocEndpointInfo = new EdhocEndpointInfo(idCreds, creds, keyPairs, peerPublicKeys,
				...
				eadProductionInput, trustModel, db, uriLocal, OSCORE_REPLAY_WINDOW, MAX_UNFRAGMENTED_SIZE, appProfiles);
	...
	/*
	 * Definition of the Light Resource
	 */
	...
	private static void setupOwnAuthenticationCredentials() {
		...
		if (supportedCiphersuites.contains(Integer.valueOf(Constants.EDHOC_CIPHER_SUITE_0))
			...
			privateKeyBinary = StringUtil
					.hex2ByteArray("df69274d713296e246306365372b4683ced5381bfcadcd440a24c391d2fedb94");
			...
			privateKeyBinary = StringUtil
					.hex2ByteArray("bb501aac67b9a95f97e0eded6b82a662934fbbfc7ad1b74c1fcad66a079422d0");
			publicKeyBinary = StringUtil
		...
		if (supportedCiphersuites.contains(Integer.valueOf(Constants.EDHOC_CIPHER_SUITE_2))
			...
			privateKeyBinary = StringUtil
					.hex2ByteArray("ec93c2f8a58f123daa982688e384f54c10c50a1d2c90c00304f648e58f14354c");
			publicKeyBinary = StringUtil
			...
			privateKeyBinary = StringUtil
					.hex2ByteArray("72cc4761dbd4c78f758931aa589d348d1ef874a7e303ede2f140dcf3e6aa4aac");
			publicKeyBinary = StringUtil
	...
	private static void setupPeerAuthenticationCredentials() {
		...
		peer1IdCredEd25519x5t = Util.buildIdCredX5t(peerSerializedCert); // ID_CRED
																			// as
																			// 'x5t'
		peer1IdCredEd25519x5u = Util.buildIdCredX5u("http://example.repo.com/hostA-x509-Ed25519"); // ID_CRED
																									// as
																									// 'x5u'
		...
		peer1IdCredX25519x5t = Util.buildIdCredX5t(peerSerializedCert); // ID_CRED
																		// as
																		// 'x5t'
		peer1IdCredX25519x5u = Util.buildIdCredX5u("http://example.repo.com/hostA-x509-X25519"); // ID_CRED
																									// as
																									// 'x5u'
		...
		peer1IdCredP256x5t = Util.buildIdCredX5t(peerSerializedCert); // ID_CRED
																		// as
																		// 'x5t'
		peer1IdCredP256x5u = Util.buildIdCredX5u("http://example.repo.com/hostA-x509-P256-signing"); // ID_CRED
																										// as
																										// 'x5u'
		...
		peer1IdCredP256DHx5t = Util.buildIdCredX5t(peerSerializedCert); // ID_CRED
																		// as
																		// 'x5t'
		peer1IdCredP256DHx5u = Util.buildIdCredX5u("http://example.repo.com/hostA-x509-P256-dh"); // ID_CRED
																									// as
																									// 'x5u'
