...
public class MqttV5Receiver implements MqttCallback {
	 ...
	 * @throws InterruptedException
	 */
	public boolean validateReceipt(String sendTopic, int expectedQos, MqttMessage message)
	 ...
	 * @throws InterruptedException
	 */
	public boolean validateReceipt(String sendTopic, int expectedQos, byte[] sentBytes)
			throws MqttException, InterruptedException {
	 ...
	 * publishers The message payloads are expected to have the format<b> "Batch
	 * Message payload :<batch>:<publisher>:<messageNumber>:<any additional
	 * payload>"
	 * 
	 ...
	 * @throws InterruptedException
	 */
	public boolean validateReceipt(List<String> sendTopics, List<Integer> expectedQosList, int expectedBatchNumber,
			int nPublishers, List<byte[]> sentBytes, boolean expectOrdered) throws MqttException, InterruptedException {
		final String methodName = "validateReceipt";
		log.entering(className, methodName, new Object[] { sendTopics, expectedQosList, sentBytes });
			...
			if (expectOrdered) {
				try {
					...
					if (messageNumber == expectedMessageNumbers[publisher]) {
						...
						report("Received invalid payload\n" + "Received:" + payloadString);
						report("messageNumber " + messageNumber + " was received out of sequence - expected value was "
		...
		log.exiting(className, methodName, new Object[] { "return true" });
		return true;
	}
	...
	public synchronized boolean waitForConnectionLost(long waitMilliseconds) throws InterruptedException {
		...
		log.entering(className, methodName, new Object[] { waitMilliseconds, connected });

		if (connected) {
			wait(waitMilliseconds);
		}

		log.exiting(className, methodName, new Object[] { connected });
		return connected;
	}
	...
	public void connectionLost(Throwable cause) {
		...
		log.entering(className, methodName, new Object[] { cause, connected });
	...
	public synchronized void messageArrived(String topic, MqttMessage message) throws Exception {
		...
		log.entering(className, methodName, new Object[] { topic, message });
